...
public record EditEmailTemplateCommand(string EmailTemplateId, string Name, string Description, string Subject, string Body, bool Active, string Category, string UpdatedBy)
...
internal class EditEmailTemplateHandler : ICommandHandler<EditEmailTemplateCommand, EditEmailTemplateResult>
{
    ...
    public async Task<EditEmailTemplateResult> Handle(EditEmailTemplateCommand command, CancellationToken cancellationToken)
    {
        ...
        emailTemplate.UpdatedBy = command.UpdatedBy; // Sử dụng UpdatedBy từ command
