...
namespace ShoppingCart.API.Repository
{
    public class CachedCartRepository : ICartRepository
    {
        ...
        public async Task<CartHeader> CreateCartHeader(CartHeader cartHeader, CancellationToken cancellationToken = default)
        {
            ...
            var createdCartHeader = await _repository.CreateCartHeader(cartHeader, cancellationToken);
            ...
            await _cache.SetStringAsync($"cart:{cartHeader.CreatedBy}", JsonSerializer.Serialize(cartDto), cancellationToken);
        ...
        {
            var updatedHeader = await _repository.UpdateCartHeader(cartHeader, cancellationToken);
